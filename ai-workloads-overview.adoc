...
summary: 'L"usine de workloads pour GenAI vous permet d"intégrer les systèmes de fichiers Amazon FSX pour NetApp ONTAP aux modèles de base GenAI. Cela vous fournit un stockage haute performance avec un ensemble complet de fonctionnalités de protection, de sécurité et d"optimisation des coûts pour vos datasets d"IA.' 
---
= Découvrez l'usine de workloads pour GenAI
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
== Qu'est-ce que l'usine de charges de travail pour GenAI ?
...
== L'usine de workloads pour GenAI aide à créer des applications d'IA générative
