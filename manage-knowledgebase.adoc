...
. Sur la page Modifier la base de connaissances, vous pouvez modifier le nom de la base de connaissances, sa description, son modèle d'intégration, son modèle de discussion et si les amorces de conversation sont créées automatiquement ou manuellement.
+
...
Si vous n'avez plus besoin d'une base de connaissances, vous pouvez la supprimer. Lorsque vous supprimez la base de connaissances, celle-ci est supprimée de Workload Factory et toutes les applications ou chatbots qui utilisent la base de connaissances cessent de fonctionner. Cette action n'est pas réversible.

Lorsque vous supprimez une base de connaissances, vous devez également effectuer les actions suivantes pour supprimer entièrement toutes les ressources associées à la base de connaissances :

* Dissociez la base de connaissances de tous les agents avec lesquels elle est associée.
* Les données sous-jacentes qui ont été indexées à partir de votre base de connaissances restent dans le magasin vectoriel que vous avez configuré et peuvent toujours être récupérées. Pour supprimer les données, vous devez également supprimer l'index vectoriel contenant les codes de données.
...
.Résultat
La base de connaissances est supprimée de Workload Factory.
